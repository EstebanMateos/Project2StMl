...
def learn_Q(env, n_sims, gamma = 1, omega = 0.77, epsilon = 0.05,
    ...
    #######################################################################
            ...
            Q[state][action] += alpha * (action_reward + gamma*np.argmax(Q[state2]) - Q[state][action])
    ...
    for episode in range(1,n_sims + 1):
     ...
     #################################################################### 
        ...
        for state in episode_state_action_count.keys():
            for action, count in enumerate(episode_state_action_count[state]):
                if count > 0: # If the state-action pair was visited
